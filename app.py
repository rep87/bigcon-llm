...
def _build_debug_report_markdown(
    ...
    *,
    ...
    if prompt_trace.get("length_policy"):
        lines.append(f"- length_policy: {prompt_trace.get('length_policy')}")
    if prompt_trace.get("input_token_limit") is not None:
        lines.append(
            "- prompt tokens: "
            + f"est={prompt_trace.get('prompt_tokens_est')} limit={prompt_trace.get('input_token_limit')}"
        )
    if prompt_trace.get("truncation_notes"):
        lines.append(f"- truncation_notes: {prompt_trace.get('truncation_notes')}")
    ...
    if not response_trace:
        ...
        if response_trace.get("prompt_tokens_est") is not None:
            lines.append(
                "- prompt_tokens_est: "
                + f"{response_trace.get('prompt_tokens_est')} (limit={response_trace.get('input_token_limit')})"
            )
        if response_trace.get("prompt_truncation_notes"):
            lines.append(f"- prompt_truncation_notes: {response_trace.get('prompt_truncation_notes')}")
        if response_trace.get("max_output_tokens") is not None:
            lines.append(f"- max_output_tokens: {response_trace.get('max_output_tokens')}")
        if response_trace.get("prompt_over_limit"):
            lines.append("- prompt_over_limit: True")
...
def render_debug_view(agent1_json: dict | None, show_raw: bool | None = None) -> None:
    ...
    st.write({
        ...
        "rows_after": panel_info.get("rows_after"),
        "alias_allowed": debug.get("panel_alias_allowed"),
...
def _collect_major_customers(agent1_json: dict) -> str:
    ...
    if len(buckets) > 3:
        segments.append(f"+ {len(buckets) - 3}개 더")
...
length_policy_options = {
    "short": "짧게",
    "balanced": "균형",
    "long": "길게",
}
current_policy = st.session_state.get("llm_length_policy", "balanced")
if current_policy not in length_policy_options:
    current_policy = "balanced"
st.sidebar.selectbox(
    "LLM 길이 정책",
    options=list(length_policy_options.keys()),
    index=list(length_policy_options.keys()).index(current_policy),
    format_func=lambda key: length_policy_options.get(key, key),
    key="llm_length_policy",
)

data_flags["allow_alias_columns"] = st.sidebar.toggle(
    "영문화 매핑 허용",
    value=bool(data_flags.get("allow_alias_columns", False)),
)

st.session_state["_data_flags"] = data_flags
os.environ["PANEL_ALLOW_ALIASES"] = "1" if data_flags.get("allow_alias_columns") else "0"
    ...
    if run_analysis:
        ...
        with st.spinner("Agent-2: 카드 생성 중..."):
            try:
                ...
                llm_in_limit = int(st.session_state.get("llm_in_ctx", 8192))
                llm_out_limit = int(st.session_state.get("llm_out_max", 2048))
                llm_length_policy = st.session_state.get("llm_length_policy", "balanced")
                prompt_text = build_agent2_prompt(
                    a1,
                    question_text=question,
                    question_type=question_type,
                    rag_context=rag_prompt_context,
                    length_policy=llm_length_policy,
                    input_token_limit=llm_in_limit,
                )
                ...
                result = call_gemini_agent2(
                    ...
                    length_policy=llm_length_policy,
                    input_token_limit=llm_in_limit,
                    max_output_tokens=llm_out_limit,
                )
